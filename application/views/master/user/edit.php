		...
		<table class="form">
			...
			<tr>
				<td><label>password</label><br />
					...
					<input name="password" type="password" id="password" value="<?php //echo $user->password; 
																				?>" size="40" />
					<?php echo form_error('user'); ?>
				</td>
			</tr>
			...
			<tr>
				<td><label>jenis</label><br />
					<select name="jenis" id="jenis">
						<option value="1" <?= ($user->jenis == '1') ? 'selected' : '' ?>>SIMAKDA BUD</option>
						<option value="2" <?= ($user->jenis == '2') ? 'selected' : '' ?>>SIMAKDA SKPD</option>
					</select>
		...
		<table class="narrow">
			...
			?>
				<tr>
					...
					<td><input type="checkbox" name="otori_id[]" value="<?php echo $menu->id; ?>" id="<?php echo "$i" ?>" <?php
																															if ($menu->user_id == $user->id_user) {
