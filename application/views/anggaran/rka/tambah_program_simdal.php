...
/* 		  $(function(){
	...
	$("#dg").edatagrid("selectAll");
        ...
        function get_simdal(){
            ...
            $(function(){
			$('#dg').edatagrid({
				url: '<?php echo base_url(); ?>/index.php/rka/program_simdal_baru/'+jns+'/'+skpd,
                 idField:'id',
                 toolbar:"#toolbar",              
                 rownumbers:"true",
                 nowrap:false, 
	...
	$(function(){
        ...
        function simpan(){
			...
			for(var i=0;i<rows.length;i++){            
                ...
                ckdskpd     = rows[i].kd_skpd.trim();
				if(i>0){
					csql = csql+","+"('"+ckdprog+"','"+cnmprog+"','"+ckdskpd+"')";
				}else{
					csql = "values('"+ckdprog+"','"+cnmprog+"','"+ckdskpd+"')";                 
				}                                             
			}   	                  
			...
			$(document).ready(function(){
				...
				$.ajax({
					...
					dataType : 'json',                 
					data: ({sql:csql}),
					url: '<?php echo base_url(); ?>/index.php/rka/dsimpan_program',
					success:function(data){                        
						status = data.pesan;   
						 if (status=='1'){
							alert('Data Berhasil Tersimpan...!!!');
...
<div id="content">   
 <!-- <?php echo $prev; ?>-->
  ...
  <h3>K E G I A T A N &nbsp;&nbsp;&nbsp;&nbsp;<input id="giat" name="giat" style="width: 150px;" />&nbsp;&nbsp;&nbsp;<input id="nm_giat" name="nm_giat" readonly="true" style="width:650px;border: 0;"/> </h3>
   ...
   <h3><button type="button" onclick="javascript:get_simdal()">Ambil Program Simdal Baru</button>&nbsp;&nbsp;<button type="button" onclick="javascript:simpan()">Simpan Data</button></h3> 
   <table id="dg" title="Program SimdalRenbang Baru" style="width:910%;height:300%" >  </table>    	    
   <h3><button type="button" onclick="javascript:get_simdal2()">Ambil Program Simdal Edit</button>&nbsp;&nbsp;<button type="button" onclick="javascript:simpan2()">Simpan Data</button></h3> 
