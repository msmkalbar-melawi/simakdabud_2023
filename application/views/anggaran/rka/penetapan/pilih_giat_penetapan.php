    ...
    $(document).ready(function() {
		...
		$('#Xskpd').combogrid({  
			...
			$('#anggaran').combogrid({
                ...
                textField: 'nama',
                mode: 'remote',
                url: '<?php echo base_url(); ?>index.php/rka_penetapan/anggaran',
                columns: [
                    [{
                        field: 'nama',
                        title: 'Nama',
		...
		function kegiatan(skpd,urusan=''){
			$('#giat').combogrid({  
	            ...
	            mode:'remote',
	            url:'<?php echo base_url(); ?>/index.php/rka_penetapan/ld_giat_penetapan/'+skpd+'/'+urusan,  
	            columns:[[  
        ...
        function validate_combo(skpd){
			...
			$('#dg').edatagrid({
				url: '<?php echo base_url(); ?>/index.php/rka_penetapan/select_giat_penetapan/'+skpd + '/' + anggaran,
                 idField:'id',
                 toolbar:"#toolbar",              
                 rownumbers:"true", 
                 fitColumns:"true",
				...
				var apdet=1;
							...
							$(function(){   
						        $.ajax({
						           type     : "POST",
						           dataType : "json",
						           url: '<?php echo base_url(); ?>/index.php/rka_penetapan/status_giat/'+apdet+'/'+giat+'/'+skpd+'/'+anggaran,  
						           success  : function(data){
						           			$("#dg").datagrid("unselectAll");
						           			$('#dg').edatagrid('reload');
						           }
					...
					if  (del==true){
							...
							$(function(){   
						        $.ajax({
						           type     : "POST",
						           dataType : "json",
						           url: '<?php echo base_url(); ?>/index.php/rka_penetapan/ghapus_penetapan/'+skpd+'/'+giat+'/'+anggaran, 
						           success  : function(data){
						           			$("#dg").datagrid("unselectAll");
						           			$('#dg').edatagrid('reload');
...
<input type="text" hidden name="egiat" id="egiat">
  ...
  <h3>ANGGARAN &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input id="anggaran" name="anggaran" style="width:200px;border: 0;"/> </h3> 
