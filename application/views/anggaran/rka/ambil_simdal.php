...
/* 		  $(function(){
	...
	$("#dg").edatagrid("selectAll");
        ...
        function get_simdal(){
            ...
            $(function(){
			$('#dg').edatagrid({
				url: '<?php echo base_url(); ?>/index.php/rka/get_simdal_skpd/'+skpd,
                 idField:'id',
                 toolbar:"#toolbar",              
                 rownumbers:"true",
                 nowrap:false, 
                 ...
                 columns:[[
					...
					{field:'ID_SKPD',title:'Kode SKPD',width:80,align:'left',formatter:function(value,row){return '<div style="height:25px;line-height:12px">'+value+'</div>';}},
                    ...
                    {field:'SK_SKPD',title:'SK SKPD',width:120,align:'left',formatter:function(value,row){return '<div style="height:25px;line-height:12px">'+value+'</div>';}},
                    {field:'LO_SKPD',title:'Lokasi',width:100,align:'left',formatter:function(value,row){return '<div style="height:25px;line-height:12px">'+value+'</div>';}}
				]]
			});
		});
        }
	...
	$(function(){
        ...
        function simpan(){
            //var cbln = document.getElementById('kebutuhan_bulan').value;  
            var tabel = 'ms_skpd_simdal';
            var field = '(kd_skpd,nm_skpd,sk_skpd,lokasi)';           
			...
			for(var i=0;i<rows.length;i++){            
                ...
                cskskpd     = rows[i].SK_SKPD.trim();
                closkpd     = rows[i].LO_SKPD.trim();
				if(i>0){
					csql = csql+","+"('"+ckdskpd+"','"+cnmskpd+"','"+cskskpd+"','"+closkpd+"')";
				}else{
					csql = "values('"+ckdskpd+"','"+cnmskpd+"','"+cskskpd+"','"+closkpd+"')";                 
				}                                             
			}   	                  
			...
			$(document).ready(function(){
				...
				$.ajax({
					type: "POST",   
					dataType : 'json',                 
					data: ({sql:csql,tabel:tabel,field:field}),
					url: '<?php echo base_url(); ?>/index.php/rka/dsimpan_simdal',
					success:function(data){                        
						status = data.pesan;   
						 if (status=='1'){
...
<div id="content">   
 <!-- <?php echo $prev; ?>-->
  ...
  <h3>K E G I A T A N &nbsp;&nbsp;&nbsp;&nbsp;<input id="giat" name="giat" style="width: 150px;" />&nbsp;&nbsp;&nbsp;<input id="nm_giat" name="nm_giat" readonly="true" style="width:650px;border: 0;"/> </h3>
   ...
   <h3>&nbsp;&nbsp;&nbsp;&nbsp; <button type="button" onclick="javascript:get_simdal()">Ambil SKPD Simdal</button>&nbsp;&nbsp;<button type="button" onclick="javascript:simpan()">Simpan Data</button></h3> 
   <table id="dg" title="SKPD SimdalRenbang" style="width:910%;height:300%" >  
