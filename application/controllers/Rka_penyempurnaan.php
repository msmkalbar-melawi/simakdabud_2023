<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Rka_penyempurnaan extends CI_Controller {
    ...
    function tambah_rka_sempurna()
        {
            ...
            $this->template->load('template','anggaran/rka/penyempurnaan/tambah_rka_penyempurnaan',$data) ; 
            $query1->free_result();
       }
    ...
    function select_rka_penyempurnaan($kegiatan='', $skpd='') {
            ...
            $sql = "SELECT a.kd_rek6,b.nm_rek6,a.nilai,a.nilai_sempurna,a.nilai_ubah,a.sumber1_su,
            (select g.nm_sumberdana from hsumber_dana g where a.sumber1_su=g.kd_sumberdana)as nmsumber,
            a.sumber2_su,
            (select g.nm_sumberdana from hsumber_dana g where a.sumber2_su=g.kd_sumberdana)as nmsumber2,
            a.sumber3_su,
            (select g.nm_sumberdana from hsumber_dana g where a.sumber3_su=g.kd_sumberdana)as nmsumber3,
            a.sumber4_su,
            (select g.nm_sumberdana from hsumber_dana g where a.sumber4_su=g.kd_sumberdana)as nmsumber4,
            a.nsumber1_su,a.nsumber2_su
                    ,a.nsumber3_su,a.nsumber4_su from trdrka a inner join ms_rek6 b on a.kd_rek6=b.kd_rek6 join 
                    trskpd c on a.kd_sub_kegiatan=c.kd_sub_kegiatan
                    where a.kd_sub_kegiatan='$kegiatan' and a.kd_skpd='$skpd'/*and left(a.kd_rek6,4) 
                    not in ('5101') */
GROUP BY a.kd_rek6,b.nm_rek6,a.nilai,a.nilai_sempurna,a.nilai_ubah,a.sumber1_su,a.sumber2_su,a.sumber3_su,a.sumber4_su,a.nsumber1_su,a.nsumber2_su
,a.nsumber3_su,a.nsumber4_su
                    order by a.kd_rek6 ";                   
       ...
       function load_daftar_harga_detail_ck() {
            ...
            $reklancar   = substr($rekening,0,4);
                ...
                $sql    = "SELECT * from ms_standar_harga where (left(kd_rek6,2)='$rektetap' OR kd_rek6='' OR kd_rek6='$rekening') and (upper(uraian_barang) like upper('%$cari%') ) order by id_standar_harga ";
            $query1 = $this->db->query($sql);  
            $result = array();
            $ii     = 0;
            foreach($query1->result_array() as $resulte)
     ...
     function ambil_sdana(){
            ...
            $query1 = $this->db->query("SELECT a.kd_sumberdana,a.nm_sumberdana,(nilai+nilaisilpa) -
                                        (select 
                                        (select isnull(sum(nsumber1_su),0)as k from trdrka 
                                        where sumber1_su=a.kd_sumberdana and left(kd_rek6,1) <> '4' and left(kd_rek6,2) <>'61' /*and left(kd_rek6,4) <>'5101'*/)+
                                        (select isnull(sum(nsumber2_su),0)as m from trdrka 
                                        where sumber2_su=a.kd_sumberdana and left(kd_rek6,1) <> '4' and left(kd_rek6,2) <>'61' /*and left(kd_rek6,4) <>'5101'*/)+
                                        (select isnull(sum(nsumber3_su),0)as n from trdrka 
                                        where sumber3_su=a.kd_sumberdana and left(kd_rek6,1) <> '4' and left(kd_rek6,2) <>'61' /*and left(kd_rek6,4) <>'5101'*/)+
                                        (select isnull(sum(nsumber4_su),0)as v from trdrka 
                                        where sumber4_su=a.kd_sumberdana and left(kd_rek6,1) <> '4' and left(kd_rek6,2) <>'61' /*and left(kd_rek6,4) <>'5101'*/) )as sisa
    ...
    function sumber(){
            ...
            $hasil=0;
            $sql        = "SELECT
                                sumber,isnull(nsumber1_su,0)as nilai_sumber
                                from trdrka 
                                where kd_skpd='$cskpd' 
...
function tsimpan_rinci_jk_penyempurnaan_temp(){
        ...
        $sql ="INSERT into trdpo_temp(header,kode,kd_barang,kd_rek6,no_po,no_trdrka,uraian,volume1,volume2,volume3,volume_sempurna1,volume_sempurna2,volume_sempurna3,satuan1,satuan2,satuan3,satuan_sempurna1,satuan_sempurna2,satuan_sempurna3,harga,harga_sempurna1,harga_ubah,total,total_sempurna1,total_ubah)
        values ('$header','$kode','$kd_barang','$kd_rek5','$no_po','$no_trdrka','$uraian','$volume1','$volume2','$volume3','$volume1_sempurna1','$volume1_sempurna2','$volume1_sempurna3','$satuan1','$satuan2','$satuan3','$satuan_sempurna1','$satuan_sempurna2','$satuan_sempurna3','$harga1','$harga_sempurna1','$harga_ubah1','$total','$total_sempurna','$total_ubah')";
        $this->db->query($sql);
        }
 ...
 from trdpo_temp where no_trdrka='$norka'"; 
            ...
            $query1 = $this->db->query(" update trdrka set 
                                        nilai_sempurna= (select sum(total_sempurna1) as nl from trdpo where no_trdrka=trdrka.no_trdrka),
                                        nilai_ubah=(select sum(total_sempurna1) as nl from trdpo where no_trdrka=trdrka.no_trdrka),
                                        nilai_akhir_sempurna=(select sum(total_sempurna1) as nl from trdpo where no_trdrka=trdrka.no_trdrka)
                                        ,username='$id',last_update=getdate(),      
...
function rka_rinci_penyempurnaan($skpd='',$kegiatan='',$rekening='') {
        $norka  = $skpd.'.'.$kegiatan.'.'.$rekening;        
        $result = $this->rka_model->rka_rinci_penyempurnaan($skpd,$kegiatan,$rekening,$norka);
        echo json_encode($result);
    }
...
function hapus_rincian_temp(){
    ...
    function load_sum_rek_rinci_penyempurnaan(){
            ...
            $query1 = $this->db->query(" select sum(total) as rektotal_rinci,sum(total_sempurna1) as rektotal_rinci_ubah from trdpo_temp where no_trdrka='$norka' ");  
            $result = array();
            $ii = 0;
            foreach($query1->result_array() as $resulte)
            { 
