...
class cetak_rincian_objek extends CI_Controller
{
	...
	function load_ttd($ttd='', $kdskpd='')
	{
		$sql = "SELECT * FROM ms_ttd WHERE kd_skpd= '$kdskpd' and kode in ('$ttd','KPA')";
	...
	function load_ttd2($ttd='', $kdskpd='')
	{
		
		$sql = "SELECT * FROM ms_ttd WHERE kd_skpd= '$kdskpd' and kode in ('$ttd')";
	...
	function ld_giat_rinci_objek($skpd = '')
	{
		...
		$sql = "SELECT DISTINCT kd_sub_kegiatan,nm_sub_kegiatan FROM trdrka where kd_skpd='$skpd' AND jns_ang='$jns_ang' and (upper(kd_sub_kegiatan) like upper('%$lccr%') or upper(nm_sub_kegiatan) like upper('%$lccr%')) order by kd_sub_kegiatan ";
	...
	function ld_rek_rinci_objek($giat = '', $skpd='')
	{
		$lccr = $this->input->post('q');
        $jns_ang = $this->cek_anggaran_model->cek_anggaran($skpd);
		$sql = " SELECT DISTINCT kd_rek6, nm_rek6 FROM trdrka where kd_sub_kegiatan='$giat' AND jns_ang='$jns_ang' and (upper(kd_rek6) like upper('%$lccr%') or upper(nm_rek6) like upper('%$lccr%')) order by kd_rek6 ";
		$query1 = $this->db->query($sql);
		$result = array();
		$ii = 0;
		foreach ($query1->result_array() as $resulte) {
