...
class Tukd2 extends CI_Controller {
	...
	function register_pajak3($ctk='',$ctgl1='',$ctgl2='',$spasi=''){
       $lntahunang = $this->session->userdata('pcThang'); 
	   $tanggal_awal = $this->tukd_model->tanggal_format_indonesia($ctgl1);
	   $tanggal_akhir = $this->tukd_model->tanggal_format_indonesia($ctgl2);
	   
	   $cRet = '';
        $cRet .="<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"".$spasi."\" cellpadding=\"".$spasi."\">
                 <tr>
                    ...
                    <b>REGISTER PAJAK</b><BR>
                    <b>PERIODE $tanggal_awal SAMPAI DENGAN $tanggal_akhir</b>
                    <BR>&nbsp;                                        
                    </td>           
                </tr>
				</table>
			...
			$cRet .='<TABLE style="border-collapse:collapse;font-size:14px" border="1" cellspacing="'.$spasi.'" cellpadding="'.$spasi.'" width="150%" >
				...
				$query = $this->db->query("SELECT kd_skpd, (select nm_skpd from trdrka where kd_skpd=z.kd_skpd group by nm_skpd) nm_skpd
											...
											(
											SELECT b.kd_skpd
											,SUM(CASE WHEN b.kd_rek5 IN ('2130301','2110401') AND a.jns_spp IN('1','2','3') THEN b.nilai ELSE 0 END) AS ppn_up
											...
											,SUM(CASE WHEN b.kd_rek5 IN ('2130301','2110401') AND a.jns_spp IN('4','5','6') THEN b.nilai ELSE 0 END) AS ppn_ls
											...
											AND b.kd_rek5 IN ('2130301','2110401','2130101','2130201','2130401','2130501')
											WHERE a.tgl_bukti BETWEEN '$ctgl1' AND '$ctgl2'
											GROUP BY b.kd_skpd
											...
											,0 AS pph23_ls
											,0 AS pph4_ls
											,SUM(CASE WHEN b.kd_rek5 IN ('2111001','2110901') THEN b.nilai ELSE 0 END) AS ppnpn2
                                            ...
                                            ,SUM(CASE WHEN b.kd_rek5 IN ('2111201','2110201') THEN b.nilai ELSE 0 END) AS jkk
											...
											,0 as setor
											FROM trhtrmpot a INNER JOIN trdtrmpot b ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd
											AND b.kd_rek5 IN ('2111001','2110901','2111101','2110701','2110702','2110703','2110501','2111201','2110201','2111301','2111401')
											WHERE a.tgl_bukti BETWEEN '$ctgl1' AND '$ctgl2'
											GROUP BY  b.kd_skpd
											...
											AND b.kd_rek5 IN('2130301','2110401','2130101','2130201','2130401','2130501','2111001','2110901','2111101','2110701','2110702','2110703','2110501','2111201','2110201','2111301','2111401')
											WHERE a.tgl_bukti BETWEEN '$ctgl1' AND '$ctgl2'
											GROUP BY b.kd_skpd
											)a group by a.kd_skpd )z
											where kd_skpd<>''
											order by kd_skpd
											");  
				...
				foreach ($query->result() as $row) {
					...
					$cRet .='<TR>
								...
								<TD align="right" >'.number_format($terima-$setor,"2",",",".").'</TD>
							 </TR>';					
	...
	switch ($ctk)
        {
            case 0;
			 echo ("<title> Register Pajak</title>");
				echo $cRet;
				break;
            case 1;
	...
	function register_uyhd_pajak_tlalu($lcskpd='',$nbulan='',$ctk='',$ttd1='',$tgl_ctk='',$ttd2='', $spasi=''){
        ...
        $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab,pangkat FROM ms_ttd where kd_skpd='$lcskpd' and kode='PA' and nip='$ttd2'";
        ...
        $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat FROM ms_ttd where kd_skpd='$lcskpd' and kode='BK' and nip='$ttd1'";
			...
			$query = $this->db->query("SELECT sum(trm_uyhd) trm_uyhd, sum(trm_pajak) trm_pajak, sum(str_uyhd) str_uyhd, sum(str_pajak) str_pajak FROM (
							select 1 as nomor, kd_skpd, '2017-01-03' tanggal, 'Terima UYHD' uraian, sld_awal as trm_uyhd, 0 trm_pajak, 0 as str_uyhd, 0 as str_pajak from ms_skpd where kd_skpd='$lcskpd'
							UNION ALL
							select 2 as nomor, kd_skpd, '2017-01-03' tanggal, 'Terima Pajak' uraian, 0 as trm_uyhd, sld_awalpajak as  trm_pajak, 0 as str_uyhd, 0 as str_pajak  from ms_skpd where kd_skpd='$lcskpd'
							UNION ALL
	...
	function ctk_jurnal_koreksi($dcetak='',$dcetak2='',$skpd='',$tgl_ttd='',$ttd1='',$ttd2='',$spasi='',$ctk=''){
        ...
        $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat FROM ms_ttd where kd_skpd='$skpd' and nip='$ttd2' and kode='PA'";
                 $sqlttd=$this->db->query($sqlttd1);
                 foreach ($sqlttd->result() as $rowttd)
                {
                    $nip1=$rowttd->nip;                    
