...
//        
    ...
    function list_no_terima($kode) {
        // $kd_skpd = $this->session->userdata('kdskpd');
        $lccr = $this->input->post('q');
        $kode = $this->uri->segment(3);
        
        $sql   = "SELECT a.*,b.nm_sub_kegiatan,c.nm_rek6 FROM tr_terima a JOIN ms_sub_kegiatan b ON a.kd_sub_kegiatan = b.kd_sub_kegiatan JOIN ms_rek6 c ON a.kd_rek6 = c.kd_rek6 where a.kd_skpd='$kode' AND a.no_terima NOT IN(select ISNULL(no_terima,'') no_terima from trdkasin_pkd where kd_skpd='$kode') AND (a.no_terima LIKE '%$lccr%' or a.nilai LIKE '%$lccr%') ORDER BY a.tgl_terima,a.kd_rek6";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function load_trskpd_pend() {        
        ...
        if ($jenis ==4){
            ...
            $jns_beban = "and a.jns_sub_kegiatan='52'";
        }
        if ($giat !=''){                               
    ...
    function load_trskpd1_pend() {
            ...
            $sql = "SELECT a.kd_sub_kegiatan,a.nm_sub_kegiatan FROM trskpd_pend a 
        ...
        if ($tabel == 'trhkasin_pkd') {
            ...
            if ($asg){
                ...
                //  }  
                    ...
                    //  $sql = "insert into trhkasin_ppkd(no_kas,no_sts,kd_skpd,tgl_sts,tgl_kas,keterangan,total,kd_bank,kd_sub_kegiatan,
                    //     jns_trans,rek_bank,sumber,pot_khusus,no_sp2d,jns_cp,no_terima) 
                    //     values('$num','$nomor','$skpd','$tgl','$tgl','$ket','$total','$bank','$giat','$jnsrek','$rekbank','$pengirim','0','$sp2d','$jns_cp','$no_terima')";
                    // $asg = $this->db->query($sql);
                ...
                if ($asg){
                    ...
                    if(!($asg)){
                        ...
                        // }
                        /* $sql = "update a set a.kunci=1 
                                from tr_tetap a inner join tr_terima b on a.kd_skpd=b.kd_skpd and a.no_tetap=b.no_tetap
                                                and a.kd_kegiatan=b.kd_kegiatan and a.kd_rek5=b.kd_rek5 and a.kd_rek_lo=b.kd_rek_lo
                                                inner join trdkasin_pkd c on b.kd_skpd=c.kd_skpd and b.no_terima=c.no_terima 
