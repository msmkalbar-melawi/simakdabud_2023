...
class Tukd2 extends CI_Controller {
	...
	function cetak_jurnal_k(){
        ...
        $this->template->load('template','tukd/transaksi/ctk_jurnal_koreksi',$data) ; 
    }
                ...
                $lcno = $lcno + 1;
                        ...
                        if($cnovoc==$res['no_bukti']){
                            $cRet .="<tr>
                                ...
                                <td style=\"border-bottom:none;border-top:none;\">&nbsp;</td>
                                <td colspan = '2' align='center' style=\"border-bottom:none;\">".$res['kd_sub_kegiatan']."</td>
				...
				if($res['total']>0){
                        if($res['nilai']<0){
                            $cRet .="<tr>
                                ...
                                <td style=\"border-bottom:none;border-top:none;\"></td>
                                <td style=\"border-bottom:none;border-top:none;\">&nbsp;</td>
                                <td style=\"border-bottom:none;\">".substr($res['kd_sub_kegiatan'],16,2)."</td>
                                <td style=\"border-bottom:none;\">".substr($res['kd_sub_kegiatan'],19,2)."</td>
                                <td style=\"border-bottom:none;\">".substr($res['kd_rek6'],0,1)."</td>
                                <td style=\"border-bottom:none;\">".substr($res['kd_rek6'],1,1)."</td>
                                <td style=\"border-bottom:none;\">".substr($res['kd_rek6'],2,1)."</td>
                                <td style=\"border-bottom:none;\">".substr($res['kd_rek6'],3,2)."</td>
        ...
        $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat FROM ms_ttd where kd_skpd='$lcskpd' and kode='BK' and nip='$ttd1'";
			...
			$cRet .='<TABLE style="border-collapse:collapse;font-size:14px" border="1" cellspacing="'.$spasi.'" cellpadding="'.$spasi.'" width="100%" >
				...
				$query = $this->db->query("SELECT
						...
						SUM (	CASE	WHEN jns_trans IN ('1','5')	AND jns_cp = '2' AND LEFT(b.kd_rek6,4) IN ('5101','5201') THEN	b.rupiah ELSE 0	END) AS ls_peg,
						SUM (	CASE	WHEN jns_trans IN ('1','5')	AND jns_cp = '2' AND LEFT(b.kd_rek6,4) = '5202' THEN	b.rupiah ELSE 0	END) AS ls_brng,
						SUM (	CASE	WHEN jns_trans IN ('1','5')	AND jns_cp = '2' AND LEFT(b.kd_rek6,4) = '5203' THEN	b.rupiah ELSE 0	END) AS ls_modal,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) = '1101' THEN	b.rupiah ELSE 0	END) AS gu,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) IN ('5101','5201') THEN b.rupiah	ELSE 0 END) AS up_gu_peg,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) IN ('5202') THEN	b.rupiah	ELSE 0 END) AS up_gu_brng,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) IN ('5203') THEN	b.rupiah	ELSE 0 END) AS up_gu_modal,
			...
			$query = $this->db->query("	SELECT
						...
						SUM (	CASE	WHEN jns_trans IN ('1','5')	AND jns_cp = '2' AND LEFT(b.kd_rek6,4) = '5202' THEN	b.rupiah ELSE 0	END) AS ls_brng_l,
						SUM (	CASE	WHEN jns_trans IN ('1','5')	AND jns_cp = '2' AND LEFT(b.kd_rek6,4) = '5203' THEN	b.rupiah ELSE 0	END) AS ls_modal_l,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) = '1101' THEN	b.rupiah ELSE 0	END) AS gu_l,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) IN ('5101','5201') THEN b.rupiah	ELSE 0 END) AS up_gu_peg_l,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) IN ('5202') THEN	b.rupiah	ELSE 0 END) AS up_gu_brng_l,
						SUM (	CASE	WHEN jns_trans = '1'	AND jns_cp = '3' AND LEFT(b.kd_rek6,4) IN ('5203') THEN	b.rupiah	ELSE 0 END) AS up_gu_modal_l,
...
//			echo  '-'.$tox_ll;
	...
	function load_sisa_tunai_blud(){
		$kd_skpd  = $this->session->userdata('kdskpd');        
        $query1 = $this->db->query("SELECT SUM(nilai) as nilai FROM tr_terima WHERE kd_skpd='$kd_skpd' AND kd_rek6='410416010001'");  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
        { 
        ...
        if ($tabel == 'trhtransout_blud') {
            ...
            // Simpan Detail //                       
				...
				if (!($asg)){
                    ...
                    $sql = "insert into trdtransout_blud(no_bukti,no_sp2d,kd_sub_kegiatan,nm_sub_kegiatan,kd_rek6,nm_rek6,nilai,kd_skpd)"; 
                    $asg = $this->db->query($sql.$csql);
					if (!($asg)){
                       $msg = array('pesan'=>'0');
                        echo json_encode($msg);
