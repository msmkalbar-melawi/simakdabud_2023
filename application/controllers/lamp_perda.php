...
class Lamp_perda extends CI_Controller
{
	...
	function cetak_lamp_lpe($kdskpd = '', $rek3 = '', $ctk = '')
	{
		...
		$cRet = '<TABLE style="border-collapse:collapse" width="100%" border="0" cellspacing="0" cellpadding="1" align=center>
					<TR>
						<TD colspan="5" align="center" > <B>RINGKASAN LAPORAN PERUBAHAN EKUITAS <BR> TAHUN ANGGARAN <?php echo date("Y"); ?> </B></TD>
					</TR>
					</TABLE>';
	...
	function cetak_lamp_beban_lo($kdskpd = '', $rek3 = '', $ctk = '')
	{
		...
		$sql = "SELECT a.kd_urusan1 as kode ,a.nm_urusan as nama, bel_pend,
	...
	function cetak_perda_lampII_5($kdskpd = '', $rek3 = '', $ctk = '')
	{
		...
		$cRet = '<TABLE style="border-collapse:collapse;font-size:11px;font-family:Arial" width="100%" border="0" cellspacing="0" cellpadding="1" align=center>
					<TR>
						...
						<TD width="95%"  align="left" >' . $rancang . ' Peraturan Daerah KABUPATEN MELAWI<br>Nomor : <u>' . $no_pergub . '</u> Tahun 2018 <br>Tentang: Pertanggungjawaban Pelaksanaan Anggaran Pendapatan dan Belanja Daerah Tahun Anggaran ' . $lntahunang . '</TD>
					</TR>
					<tr>
					</TABLE><br/>';
		$cRet .= '<TABLE style="border-collapse:collapse" width="100%" border="0" cellspacing="0" cellpadding="1" align=center>
...
<td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" style=\"font-size:12px\"><b>RUMAH SAKIT UMUM DAERAH Dr. SOEDARSO MELAWI</b></td>
...
<td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" style=\"font-size:12px\"><b>UNIT PENGOLAHAN DATA ELEKTRONIK DINAS PERHUBUNGAN, KOMUNIKASI DAN INFORMATIKA KABUPATEN MELAWI</b></td>
...
<td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" style=\"font-size:12px\"><b>DINAS PENDAPATAN DAERAH PROVINSI KALBAR (UPPD MELAWI WILAYAH I)</b></td>
<td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" style=\"font-size:12px\"><b>DINAS PENDAPATAN DAERAH PROVINSI KALBAR ( UPPD MELAWI WILAYAH II )</b></td>
...
<td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" style=\"font-size:12px\"><b>SEKRETARIAT KOMISI PENYIARAN INDONESIA DAERAH KABUPATEN MELAWI</b></td>
...
<td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" style=\"font-size:12px\"><b>DINAS KOMUNIKASI DAN INFORMATIKA PROV. KALBAR  (UNIT PENGOLAHAN DATA ELEKTRONIK KABUPATEN MELAWI)</b></td>
	...
	function cetak_lamp_bps($bulan = '', $ctk = '', $jenis = '')
	{
		...
		$cRet = '<TABLE style="border-collapse:collapse" width="100%" border="0" cellspacing="0" cellpadding="1" align=center>
					<TR>
						...
						<BR>Pemerintah KABUPATEN MELAWI Tahun ' . $lntahunang . '
	...
	function cetak_lamp_lra_permen_spj($bulan = '', $ctk = '', $anggaran = '', $jenis = '', $kd_skpd = '')
	{
		...
		$cRet = '<TABLE style="border-collapse:collapse;font-size:11px;font-family:Arial" width="100%" border="0" cellspacing="0" cellpadding="1" align=center>
					<TR>
						...
						<TD width="40%"  align="left" >Peraturan Gubernur MELAWI <br>Nomor : ' . $no_pergub . '<br>Tanggal: ' . $tgl_pergub . ' </TD>
					</TR>
					<tr>
					</TABLE><br/>';
		$cRet .= "<TABLE style=\"border-collapse:collapse;font-family:Arial\" width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
...
,'52227') THEN (debet-kredit) ELSE 0 END) as real_brng
	...
	function cetak_lamp_semester_djpk_spj_periode($tgl1 = '', $tgl2 = '', $ctk = '', $anggaran = '', $jenis = '', $kd_skpd = '', $tglttd = '', $ttd = '', $format = '')
	{
		$lntahunang = $this->session->userdata('pcThang');
		$lctgl1 = $tgl1;
		$lctgl2 = $tgl2;
		...
		if ($format == 1) {
			$map_lra = "map_lra_sap_baru_kemenkeu";
		} else {
	...
	function cetak_lamp_semester_sap_sp2d($bulan = '', $ctk = '', $anggaran = '', $jenis = '', $kd_skpd = '', $tglttd = '', $ttd = '', $format = '')
	{
		...
		$sql = "SELECT 
					...
					SUM(CASE WHEN kd_rek='4' THEN (nilai) ELSE 0 END) - SUM(CASE WHEN kd_rek='5' THEN (nilai) ELSE 0 END) as nil_surplus
					FROM
					(SELECT LEFT(kd_rek6,1) as kd_rek, SUM(nil_ang) as nil_ang, SUM(nilai) as nilai FROM $tabel($bulan,'$anggaran',$lntahunang) WHERE LEFT(kd_rek6,1) IN ('4','5') $where
					GROUP BY LEFT(kd_rek6,1)) a;
					";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "SELECT 
					...
					SUM(CASE WHEN kd_rek='61' THEN (nilai) ELSE 0 END) - SUM(CASE WHEN kd_rek='62' THEN (nilai) ELSE 0 END) as nil_netto
					FROM
					(SELECT LEFT(kd_rek6,2) as kd_rek, SUM(nil_ang) as nil_ang, SUM(nilai) as nilai FROM $tabel($bulan,'$anggaran',$lntahunang) WHERE LEFT(kd_rek6,2) IN ('61','62') $where
					GROUP BY LEFT(kd_rek6,2)) a;
					";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
...
ON a.kode=b.kode
	...
	function cetak_lamp_semester_rinci_unit_spj($bulan = '', $ctk = '', $anggaran = '', $kd_skpd = '', $jenis = '', $jenis2 = '', $tglttd = '', $ttd = '', $spj = '', $format = '')
	{
		// echo ($jenis2);
		...
		$cRet .= "<table style=\"border-collapse:collapse;font-family:Arial;font-size:11px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\">
				...
				<tr>
                    ...
                    <td width=\"32%\" align=\"center\" bgcolor=\"#CCCCCC\" ><b>URAIAN</b></td>
                    <td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" ><b>JUMLAH ANGGARAN</b></td>
                    <td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" ><b>REALISASI <br>$jdl<br> $judul</b></td>
                    <td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" ><b>SISA ANGGARAN</b></td>
                    <td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" ><b>PROGNOSIS</b></td>
		...
		$sql = "SELECT kd_sub_kegiatan,kd_rek,nm_rek,anggaran, $realisasi as sd_bulan_ini FROM realisasi_jurnal_pend_n$sumber_data($bulan,'$anggaran',$lntahunang) $where $where1 AND 
					kd_sub_kegiatan<>'' ORDER BY kd_sub_kegiatan,kd_rek";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
			...
			switch ($leng) {
				case 2:
					$cRet .= '<tr>
							   <td align="left" valign="top">' . $kd_sub_kegiatan . '.' . $this->support->dotrek($kd_rek) . '</td> 
				...
				default:
					$cRet .= '<tr>
							   <td align="left" valign="top"><b>' . $kd_sub_kegiatan . '.' . $this->support->dotrek($kd_rek) . '</b></td> 
		...
		$sql = "SELECT SUM(anggaran) anggaran ,SUM($realisasi) as sd_bulan_ini FROM realisasi_jurnal_pend_n($bulan,'$anggaran',$lntahunang) $where AND LEN(kd_rek)='$jenis' ";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "
						SELECT SUM(anggaran) anggaran ,SUM($realisasi) as sd_bulan_ini 
						FROM realisasi_jurnal_rinci_n($bulan,'$anggaran',$lntahunang) $where  AND LEN(kd_rek)='4' and urut='4' and left(kd_rek,1)in('5','6')
					";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		/*$sql="SELECT urut,kd_sub_kegiatan,kd_rek,nm_rek,anggaran,sd_bulan_ini FROM realisasi_jurnal_rinci_n$sumber_data1($bulan,$anggaran,$lntahunang) $where $where1 
						  AND SUBSTRING(kd_sub_kegiatan,17,2)='00' ORDER BY kd_sub_kegiatan,kd_rek";
					$hasil = $this->db->query($sql);
                    foreach ($hasil->result() as $row){
					   $urut = $row->urut;
					   $kd_sub_kegiatan = $row->kd_sub_kegiatan;
		...
		/*$sql="SELECT '1' kd_rek,'BELANJA LANGSUNG' nm_rek,SUM(anggaran) anggaran,SUM(sd_bulan_ini) sd_bulan_ini
						  FROM realisasi_jurnal_rinci_n$sumber_data1$conv($bulan,$anggaran,$lntahunang) $where 
							AND kd_rek IN('521','522','523','514')
							UNION ALL
		...
		$sql = "SELECT urut,kd_sub_kegiatan,kd_rek,nm_rek,anggaran,$realisasi as sd_bulan_ini  FROM realisasi_jurnal_rinci_n($bulan,'$anggaran',$lntahunang) $where $where1 
						  AND SUBSTRING(kd_sub_kegiatan,17,2)!='00' ORDER BY kd_sub_kegiatan,kd_rek";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
			$urut = $row->urut;
			...
			switch ($urut) {
				...
				case 4:
					$cRet .= '<tr>
							   <td align="left" valign="top">' . $kd_sub_kegiatan_potong . '.' . $this->support->dotrek($kd_rek) . '</td> 
							   <td align="left"  valign="top">' . $nm_rek . '</td> 
							   <td align="right" valign="top">' . number_format($nil_ang, "2", ",", ".") . '</td> 
							   <td align="right" valign="top">' . number_format($sd_bulan_ini, "2", ",", ".") . '</td> 
							   <td align="right" valign="top">' . $a . ' ' . number_format($sisa1, "2", ",", ".") . ' ' . $b . '</td> 
	...
	function cetak_lamp_semester_rinci_unit_sp2d($bulan = '', $ctk = '', $anggaran = '', $kd_skpd = '', $jenis = '', $tglttd = '', $ttd = '', $spj = '')
	{
		...
		$sql = "SELECT kd_sub_kegiatan,kd_rek,nm_rek,anggaran,sd_bulan_ini FROM realisasi_jurnal_pend_n$sumber_data($bulan,'$anggaran',$lntahunang) $where $where1 ORDER BY kd_sub_kegiatan,kd_rek";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "SELECT kd_sub_kegiatan,kd_rek,nm_rek,nil_ang anggaran,nilai sd_bulan_ini FROM $tabel($bulan,'$anggaran',$lntahunang) $where $where1 
						 AND SUBSTRING(kd_sub_kegiatan,17,5)='00.51' ORDER BY kd_sub_kegiatan,kd_rek";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
			$kd_kegiatan = $row->kd_sub_kegiatan;
			$kd_rek = $row->kd_rek;
	...
	function cetak_lamp_semester_rinci_org_sp2d($bulan = '', $ctk = '', $anggaran = '', $kd_skpd = '', $jenis = '', $tglttd = '', $ttd = '', $spj = '')
	{
		...
		$sql = "SELECT RIGHT(kd_sub_kegiatan,5) kd_kegiatan,kd_rek,nm_rek, SUM(anggaran) anggaran, SUM(sd_bulan_ini) sd_bulan_ini 
					  FROM realisasi_jurnal_pend_n$sumber_data($bulan,'$anggaran',$lntahunang) $where $where1 
					  GROUP BY RIGHT(kd_sub_kegiatan,5) ,kd_rek,nm_rek
					  ORDER BY RIGHT(kd_sub_kegiatan,5),kd_rek";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
	...
	function cetak_lamp_semester_rinci_all_spj($bulan = '', $ctk = '', $anggaran = '', $kd_skpd = '', $jenis = '', $tglttd = '', $ttd = '', $spj = '', $jenis2 = '', $tw = '', $format = '')
	{
		...
		$sql = "SELECT kd_rek,nm_rek,SUM(anggaran) anggaran, SUM($isi) sd_bulan_ini 
					 FROM realisasi_jurnal_pend_n($bulan,'$anggaran',$lntahunang) WHERE LEN(kd_rek)<='$jenis' 
					 GROUP BY kd_rek,nm_rek
					 ORDER BY kd_rek";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "SELECT SUM(anggaran) anggaran ,SUM($isi) sd_bulan_ini FROM realisasi_jurnal_pend_n($bulan,'$anggaran',$lntahunang) $where WHERE LEN(kd_rek)='$jenis' ";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "SELECT SUM(anggaran) anggaran ,SUM($isi) sd_bulan_ini FROM realisasi_jurnal_rinci_all_n($bulan,'$anggaran',$lntahunang) $where WHERE LEN(kd_rek)='$jenis' AND LEFT(kd_rek,1)<>'6' ";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {

			$nil_ang1 = round($row->anggaran);
			$sd_bulan_ini1 = $row->sd_bulan_ini;
			$sisa1 = $nil_ang1 - $sd_bulan_ini1;
			$persen1 = empty($nil_ang1) || $nil_ang1 == 0 ? 0 : $sd_bulan_ini1 / $nil_ang1 * 100;
			$sisa11 = $sisa1 < 0 ? $sisa1 * -1 : $sisa1;
		...
		foreach ($hasil->result() as $row) {
			...
			$anggaran1 = round($row->anggaran1);
			$sd_bulan_ini1 = $row->sd_bulan_ini1;
			$anggaran2 = round($row->anggaran2);
			$sd_bulan_ini2 = $row->sd_bulan_ini2;
		...
		foreach ($hasil->result() as $row) {
			...
			$sisa_ang3 = $sisa_ang * -1;
			// $sisa_ang2 = $sisa_ang < 0 ? '('.$sisa_ang3.')' : $sisa_ang;
			if ($sisa_ang < 0) {
				$sisa_ang1 = $sisa_ang * -1;
				$f = '(';
				$t = ')';
			} else {
				$sisa_ang1 = $sisa_ang;
			...
			$cRet .= '<tr>
							   ...
							   <td align="right" valign="top"><b>' . $f . '' . number_format($sisa_ang1, "2", ",", ".") . '' . $t . '</b></td> 
							   <td align="right" valign="top"><b>' . $f . '' . number_format($sisa_ang1, "2", ",", ".") . '' . $t . '</b></td> 
							   <td align="right" valign="top"><b>' . number_format($persen, "2", ",", ".") . '</b></td> 
							</tr>';
		}
		...
		$sql = "SELECT sum(case when kd_rek='61' then anggaran else 0 END) as anggaran1,
							sum(case when kd_rek='61' then sd_bulan_ini else 0 END) as sd_bulan_ini1,
							sum(case when kd_rek='62' then anggaran else 0 END) as anggaran2,
							sum(case when kd_rek='62' then sd_bulan_ini else 0 END) as sd_bulan_ini2
							 from (
							SELECT '61' kd_rek, SUM(anggaran) anggaran ,SUM($isi) sd_bulan_ini FROM realisasi_jurnal_rinci_all_n$sumber_data1$triw($bulan,'$anggaran',$lntahunang) WHERE LEN(kd_rek)='2' AND LEFT(kd_rek,2)='61'
							UNION ALL
							SELECT '62' kd_rek, SUM(anggaran) anggaran ,SUM($isi) sd_bulan_ini FROM realisasi_jurnal_rinci_all_n$sumber_data1$triw($bulan,'$anggaran',$lntahunang) WHERE LEN(kd_rek)='2' AND LEFT(kd_rek,2)='62') a";
	...
	function cetak_lamp_semester_djpk_spj($bulan = '', $ctk = '', $anggaran = '', $jenis = '', $kd_skpd = '', $tglttd = '', $ttd = '', $format = '')
	{
		$lntahunang = $this->session->userdata('pcThang');
		switch ($bulan) {
			case  1:
		...
		if ($kd_skpd == '-') {
			...
			$where = "AND LEFT(kd_skpd,17)='$kd_skpd' AND kd_skpd<>'4.02.02.02'";
			$panjang = 7;
		} else if (strlen($kd_skpd) == '17') {
			$where = "AND kd_org='$kd_skpd' AND kd_skpd<>'4.02.02.02'";
			$panjang = 17;
		} else {
			$panjang = strlen($kd_skpd);
		...
		$sql = "SELECT 
					...
					SUM(CASE WHEN kd_rek='4' THEN (kredit-debet) ELSE 0 END) - SUM(CASE WHEN kd_rek='5' THEN (debet-kredit) ELSE 0 END) as nil_surplus
					FROM
					(SELECT LEFT(kd_rek6,1) as kd_rek, SUM(nilai) as nil_ang, SUM(kredit) as kredit,SUM(debet) as debet FROM data_jurnal_n$sumber_data1($bulan,'$anggaran',$lntahunang) WHERE LEFT(kd_rek6,1) IN ('4','5') $where
					GROUP BY LEFT(kd_rek6,1)) a;
					";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "SELECT 
					...
					SUM(CASE WHEN kd_rek='61' THEN (kredit-debet) ELSE 0 END) - SUM(CASE WHEN kd_rek='62' THEN (debet-kredit) ELSE 0 END) as nil_netto
					FROM
					(SELECT LEFT(kd_rek6,2) as kd_rek, SUM(nilai) as nil_ang, SUM(kredit) as kredit,SUM(debet) as debet FROM data_jurnal_n$sumber_data1($bulan,'$anggaran',$lntahunang) WHERE LEFT(kd_rek6,2) IN ('61','62') $where
					GROUP BY LEFT(kd_rek6,2)) a;
					";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		foreach ($hasil->result() as $row) {
			...
			$sql = "SELECT SUM(nilai) as nil_ang, SUM($jenis) as nilai FROM data_jurnal_n$sumber_data1($bulan,'$anggaran',$lntahunang) 
						WHERE (
						LEFT(kd_rek6,1) IN ($kode1) or 
						LEFT(kd_rek6,2) IN ($kode2) or 
						LEFT(kd_rek6,4) IN ($kode3) or 
	...
	function cetak_lamp_semester_sap_sp2d_periode($tgl1 = '', $tgl2 = '', $ctk = '', $anggaran = '', $jenis = '', $kd_skpd = '', $tglttd = '', $ttd = '', $format = '')
	{
		$lntahunang = $this->session->userdata('pcThang');
		$lctgl1 = $tgl1;
		$lctgl2 = $tgl2;
		...
		if ($jenis == 2) {
			$tabel = 'data_sp2d_n_periode';
		} else if ($jenis == 3) {
			$tabel = 'data_sp2d_lunas_n_periode';
		} else if ($jenis == 4) {
			$tabel = 'data_sp2d_advice_n_periode';
		}
		...
		$sql = "SELECT 
						...
						SUM(CASE WHEN kd_rek='4' THEN (nilai) ELSE 0 END) - SUM(CASE WHEN kd_rek='5' THEN (nilai) ELSE 0 END) as nil_surplus
						FROM
						(SELECT LEFT(kd_rek6,1) as kd_rek, SUM(nil_ang) as nil_ang, SUM(nilai) as nilai FROM $tabel('$tgl1','$tgl2','$anggaran',$lntahunang) WHERE LEFT(kd_rek6,1) IN ('4','5') $where
						GROUP BY LEFT(kd_rek6,1)) a;
						";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "SELECT 
						...
						SUM(CASE WHEN kd_rek='61' THEN (nilai) ELSE 0 END) - SUM(CASE WHEN kd_rek='62' THEN (nilai) ELSE 0 END) as nil_netto
						FROM
						(SELECT LEFT(kd_rek6,2) as kd_rek, SUM(nil_ang) as nil_ang, SUM(nilai) as nilai FROM $tabel('$tgl1','$tgl2','$anggaran',$lntahunang) WHERE LEFT(kd_rek6,2) IN ('61','62') $where
						GROUP BY LEFT(kd_rek6,2)) a;
						";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		foreach ($hasil->result() as $row) {
			...
			$sql = "SELECT SUM(nil_ang) as nil_ang, SUM(nilai) as nilai FROM $tabel('$tgl1','$tgl2','$anggaran',$lntahunang)
							where (
							LEFT(kd_rek6,1) IN ($kode1) or 
							LEFT(kd_rek6,2) IN ($kode2) or 
							LEFT(kd_rek6,4) IN ($kode3) or 
	...
	function cetak_lamp_semester_sap_spj($bulan = '', $ctk = '', $anggaran = '', $jenis = '', $kd_skpd = '', $tglttd = '', $ttd = '', $format = '')
	{
		// echo($anggaran);
		$lntahunang = $this->session->userdata('pcThang');
		switch ($bulan) {
			...
			switch ($spasi) {
				...
				case 66:
					$cRet .= '<tr>
						   <td align="left" valign="top"><b>' . $kode . '</b></td> 
						   <td align="left"  valign="top"><b>' . $nama . '</b></td> 
						   <td align="right" valign="top"><b>' . number_format($ang_netto1, "2", ",", ".") . '</b></td> 
						   <td align="right" valign="top"><b>' . number_format($nilai, "2", ",", ".") . '</b></td> 
						   <td align="right" valign="top"><b>' . $x . '' . number_format($sisa_netto1, "2", ",", ".") . '' . $y . '</b></td> 
						   <td align="right" valign="top"><b>' . $x . '' . number_format($sisa_netto1, "2", ",", ".") . '' . $y . '</b></td> 
						   <td align="right" valign="top"><b>' . number_format($persen_netto, "2", ",", ".") . '' . $y . '</b></td> 
						</tr>';
					break;
	...
	function cetak_lamp_semester_permen_spj($bulan = '', $ctk = '', $anggaran = '', $jenis = '', $kd_skpd = '', $tglttd = '', $ttd = '')
	{
		...
		$sql = "SELECT 
					...
					FROM
					(SELECT LEFT(kd_ang,1) as kd_rek, SUM(nilai_ang) as nil_ang, SUM(real_spj) as real_spj FROM Data_realisasi_keg4($bulan, $lntahunang) WHERE jns_ang='$anggaran' and LEFT(kd_ang,1) IN ('4','5') $where
		...
		$sql = "SELECT 
					...
					FROM
					(SELECT LEFT(kd_ang,2) as kd_rek, SUM(nilai_ang) as nil_ang, SUM(real_spj) as real_spj FROM Data_realisasi_keg4($bulan,$lntahunang) WHERE jns_ang='$anggaran' and LEFT(kd_ang,2) IN ('61','62') $where
		...
		foreach ($hasil->result() as $row) {
			...
			$sql = "SELECT SUM(nilai_ang) as nil_ang, SUM(real_spj) as nilai 
					FROM Data_realisasi_keg4($bulan, $lntahunang) 
					WHERE jns_ang='$anggaran' and (LEFT(kd_ang,1) IN ($kode1) or LEFT(kd_ang,2) IN ($kode2) or LEFT(kd_ang,3) IN ($kode3) or LEFT(kd_ang,5) IN($kode4)) $where
					";
	...
	function cetak_lamp_semester_rinci_org_spj($bulan = '', $ctk = '', $anggaran = '', $kd_skpd = '', $jenis = '', $tglttd = '', $ttd = '', $spj = '', $js = '', $format = '', $jns2 = '', $tw = '')
	{
		$lntahunang = $this->session->userdata('pcThang');
		if ($jns2 == '1') {
			$isi = "sd_bulan_ini";
		...
		if ($js == '1') {
			$where = "WHERE left(kd_skpd,17)=left('$kd_skpd',17) AND kd_skpd<>'4.02.02.02'";
		} else {
			$where = "WHERE kd_skpd='$kd_skpd' AND kd_skpd<>'4.02.02.02'";
		}
		...
		$cRet .= "<TABLE style=\"border-collapse:collapse;font-family:Arial;font-size:12px\" width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
					...
					<tr>
					<td align=\"left\" style=\"border-right:hidden;border-bottom:hidden\"> &nbsp;&nbsp; Unit Organisasi </td>
					<td align=\"left\" style=\"border-left:hidden;border-bottom:hidden\"> : " . $this->left($kd_skpd, 7) . " - " . $this->tukd_model->get_nama($this->left($kd_skpd, 17), 'nm_org', 'ms_organisasi', 'kd_org') . "</td>
					</tr>
		...
		$sql = "SELECT kd_sub_kegiatan,kd_rek,nm_rek,SUM(anggaran) anggaran, SUM($isi) sd_bulan_ini 
					 FROM realisasi_jurnal_pend_n($bulan,'$anggaran',$lntahunang) $where $where1 and kd_sub_kegiatan<>'' 
		...
		$sql = "SELECT SUM(anggaran) anggaran ,SUM($isi) sd_bulan_ini FROM realisasi_jurnal_pend_n($bulan,'$anggaran',$lntahunang) $where AND LEN(kd_rek)='$jenis' ";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		$sql = "SELECT SUM(anggaran) anggaran ,SUM($isi) sd_bulan_ini FROM realisasi_jurnal_rinci_n($bulan,'$anggaran',$lntahunang) $where AND LEN(kd_rek)='$jenis' ";
		$hasil = $this->db->query($sql);
		foreach ($hasil->result() as $row) {
		...
		/*$sql="SELECT urut,SUBSTRING(kd_kegiatan,17,5) kd_kegiatan,kd_rek,nm_rek,SUM(anggaran) anggaran,SUM(sd_bulan_ini) sd_bulan_ini 
					      FROM realisasi_jurnal_rinci_n$sumber_data1($bulan,$anggaran,$lntahunang) $where $where1
		...
		$sql = "SELECT urut, kd_sub_kegiatan,kd_rek,nm_rek,SUM(anggaran) anggaran,SUM($isi) sd_bulan_ini 
					      FROM realisasi_jurnal_rinci_n($bulan,'$anggaran',$lntahunang) $where $where1
						  AND SUBSTRING(kd_sub_kegiatan,17,2)!='00' AND anggaran<>0
