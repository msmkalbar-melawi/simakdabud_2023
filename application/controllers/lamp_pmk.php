...
class Lamp_pmk extends CI_Controller
{
	...
	function  dotrek($rek)
	{
		...
		switch ($nrek) {
			...
			case 12:
				...
				break;
			case 35:
				$rek = $this->left($rek, 12) . '.' . substr($rek, 12, 0) . '' . substr($rek, 13, 1) . '.' . substr($rek, 15, 2) . '.' . substr($rek, 18, 1) . '.' . substr($rek, 20, 2) . '.' . substr($rek, 23, 1) . '.' . substr($rek, 25, 2) . '.' . substr($rek, 28, 2) . '.' . substr($rek, 31, 4);
				break;
			default:
				$rek = "";
		}
	...
	function cetak_real_pend($bulan = '', $ctk = '', $anggaran = '', $kd_skpd = '', $jenis = '', $tglttd = '', $ttd = '')
	{
		...
		$tanggal = $tglttd == '-' ? '' : 'Kabupaten Melawi, ' . $this->tukd_model->tanggal_format_indonesia($tglttd);
		if ($ttd == '-') {
			$nama = '';
			$pangkat = '';
			$jabatan = '';
			...
			$sqlsc = "SELECT TOP 1 nama as nm,nip as nip,jabatan as jab,pangkat FROM ms_ttd where kode in ('BUD','PA') and nip='$ttd'";
		...
		$sql = "SELECT kd_rek,nm_rek,anggaran,bulan_lalu,bulan_ini,sd_bulan_ini FROM penerimaan_kasda($bulan,'$anggaran') 
						WHERE LEFT(kd_rek,1)='4' AND LEFT(kd_rek,4)!='4101' AND $where and left(kd_rek,6)!='410416'
						UNION ALL
						SELECT kd_rek,nm_rek,anggaran,bulan_lalu,bulan_ini,sd_bulan_ini FROM penerimaan_kasda($bulan,'$anggaran') 
						WHERE LEFT(kd_rek,1)='4' AND LEFT(kd_rek,4)='4101' AND LEN(kd_rek)<'$jenis' and left(kd_rek,6)!='410416'
						UNION ALL
						...
						SUM(sd_bulan_ini) sd_bulan_ini FROM penerimaan_kasda_unit($bulan,'$anggaran') WHERE LEN(kd_rek)='12' AND LEFT(kd_rek,4)!='4101' AND LEFT(kd_rek,6)!='410415'  and left(kd_rek,6)!='410416'
						GROUP BY LEFT(kd_rek,$jenis), kd_skpd) a
						LEFT JOIN ms_skpd b ON a.kd_skpd=b.kd_skpd
						UNION ALL
						SELECT a.kd_rek+'.'+a.kd_skpd as kd_rek, b.nm_skpd as nm_rek, anggaran, bulan_lalu, bulan_ini, sd_bulan_ini
						FROM (SELECT LEFT(kd_rek,12) kd_rek, kd_skpd, SUM(anggaran) anggaran, SUM(bulan_lalu) bulan_lalu, SUM(bulan_ini) bulan_ini, 
						SUM(sd_bulan_ini) sd_bulan_ini FROM penerimaan_kasda_unit($bulan,'$anggaran') WHERE LEN(kd_rek)='12' AND LEFT(kd_rek,4)='4101' and left(kd_rek,6)!='410416'
						GROUP BY LEFT(kd_rek,12), kd_skpd) a
						LEFT JOIN ms_skpd b ON a.kd_skpd=b.kd_skpd
						ORDER BY kd_rek";
	...
	function cetak_real_sp2d($tgl1 = '', $tgl2 = '', $anggaran = '', $ctk = '')
	{
		...
		$cRet .= "<TABLE style=\"border-collapse:collapse;font-family:Arial\" width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
					<tr><td colspan =\"8\" align=\"center\" style=\"border-left:hidden;border-bottom:hidden\"><strong>PEMERINTAH KABUPATEN MELAWI </strong></td></tr>
