...
class Mapping extends CI_Controller {
    ...
    function ambil_skpd_mapping()
    {
        ...
        $sql = "SELECT a.* from ambil_skpd_mapping a inner join ms_skpd b on a.kd_skpd90=b.kd_skpd 
                where (validasi_kegiatan ='' OR validasi_kegiatan IS NULL) 
...
function validasi_indikator()
    ...
    function map_rekening_akuntansi()
    {
        $data['page_title'] = 'INPUT MAPPING REKENING AKUNTANSI';
        $this->template->set('title', 'INPUT MAPPING REKENING AKUNTANSI');
        $this->template->load('template', 'mapping/map_rekening_akuntansi', $data);
    }
    ...
    function get_rekening6()
    {
        ...
        $sql = "SELECT * FROM ambil_rekening6 where /*left(kd_rek6,1) in ('4','5','6') and */  (upper(kd_rek6) like upper('%$lccr%') or upper(nm_rek6) like upper('%$lccr%')) order by kd_rek6";
        ...
        foreach ($query1->result_array() as $resulte)
        {
            ...
            $result[] = array(
                ...
                'kd_rek64' => $resulte['kd_rek64'], 
                'nm_rek64' => $resulte['nm_rek64'],
                'kd_lo' => $resulte['map_lo'], 
                'nm_lo' => $resulte['lo'],
                'kd_piu' => $resulte['piutang_utang'], 
                'nm_piu' => $resulte['piu']
                 );
            $ii++;
        }
    ...
    function get_rekening64()
    {

            $lccr = $this->input->post('q'); 
            $kode = $this->input->post('kode');    

        $sql = "SELECT kd_rek64,nm_rek64 FROM ms_rek5_13 where kd_rek64<>'' and (upper(kd_rek64) like upper('%$lccr%') or upper(nm_rek64) like upper('%$lccr%')) order by kd_rek64";
     ...
     function get_rekening90lo()
    {
        ...
        $sql = "SELECT * FROM ambil_rekening90 where left(kd_rek6,1) in ('7','8','9') and (upper(kd_rek6) like upper('%$lccr%') or upper(nm_rek6) like upper('%$lccr%')) order by kd_rek6";
     ...
     function get_rekening90piu()
    {
        ...
        $sql = "SELECT * FROM ambil_rekening90 where left(kd_rek6,1) in ('1','2') and (upper(kd_rek6) like upper('%$lccr%') or upper(nm_rek6) like upper('%$lccr%')) order by kd_rek6";
...
function load_mapping_rekening()
    ...
    function load_mapping_rekening_akuntansi()
    {
        $result = array();
        $row = array();
        $page = isset($_POST['page']) ? intval($_POST['page']) : 1;
        ...
        if ($kriteria <> '')
        {
            $where = "and (upper(kd_rek6) like upper('%$kriteria%') or nm_rek6 like upper('%$kriteria%')) ";
        }

        $sql = "SELECT count(*) as total from ms_rek6";
        $query1 = $this->db->query($sql);
        $total = $query1->row();
        $result["total"] = $total->total;
        $query1->free_result();
        ...
        $sql = " 
            SELECT TOP $rows * from ms_rek6 where /* left(kd_rek6,1) in ('4','5','6') and */ kd_rek6 not in (SELECT TOP  $offset kd_rek6 from ms_rek6) $where  order by kd_rek6
        ";
        $query1 = $this->db->query($sql);
        $ii = 0;
        foreach ($query1->result_array() as $resulte)
