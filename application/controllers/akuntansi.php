...
*/
		...
		$nawal = 0;
		// surpulus operasi
        $queryOperasi = "SELECT ISNULL(ABS(SUM(CASE WHEN LEFT(kode_rekening,1) = 7 THEN kredit-debet ELSE 0 END )),0) - ISNULL(ABS(SUM(CASE WHEN LEFT(kode_rekening,1) = 8 THEN debet-kredit ELSE 0 END )),0) AS nilai FROM transaksi_lo WHERE MONTH(tanggal) <= ? AND YEAR(tanggal) = ? AND LEFT(kode_rekening,2) != 83 AND kode_skpd = ?";
        $resultOperasiTahunIni = $this->db->query($queryOperasi,[$bulan, $thn_ang,$kd_skpd])->row();
        $resultOperasiTahunLalu =  $this->db->query($queryOperasi,[12, $thn_ang-1,$kd_skpd])->row();
        $surplusTahunIni = $resultOperasiTahunIni ? $resultOperasiTahunIni->nilai : 0;
        $surplusTahunLalu = $resultOperasiTahunLalu ? $resultOperasiTahunLalu->nilai : 0;
        $kenaikanSurplus = $surplusTahunIni-$surplusTahunLalu;
        ...
        $queryNonOperasi = "SELECT 
                ISNULL(
                    ...
                    CASE WHEN LEFT(kode_rekening,1) = 8 THEN ABS(SUM(debet-kredit)) ELSE 0 END
                ,0) AS nilai
            FROM transaksi_lo WHERE MONTH(tanggal) <= ? AND YEAR(tanggal) = ? AND LEFT(kode_rekening,2) IN (74,83) AND kode_skpd = ?
            GROUP BY LEFT(kode_rekening,1)
        ";
        $resultNonOperasiTahunIni = $this->db->query($queryNonOperasi,[$bulan, $thn_ang,$kd_skpd])->row();
        $resultNonOperasiTahunLalu = $this->db->query($queryNonOperasi,[12, $thn_ang-1,$kd_skpd])->row();
        $nonOperasiTahunIni = $resultNonOperasiTahunIni ? $resultNonOperasiTahunIni->nilai : 0;
        $nonOperasiTahunLalu = $resultNonOperasiTahunLalu ? $resultNonOperasiTahunLalu->nilai : 0;

        // selisih surplus non operasional dan defisit operasion
        $surplusDefisit = $surplusTahunIni + $nonOperasiTahunIni;
        $surplusDefisitLalu = $surplusTahunLalu + $nonOperasiTahunLalu;
...
*/
			switch ($kd_rek) {
				...
				case 2:
					$cRet .= "<tr>
                                                      <td valign=\"top\"  width=\"5%\" align=\"center\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">$kd_rek</td>
                                                      <td valign=\"top\"  width=\"65%\"  align=\"left\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">$nama</td>
                                                      <td valign=\"top\"  width=\"15%\" align=\"right\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">". formatPositif($surplusDefisit) ." </td>
                                                      <td valign=\"top\"  width=\"15%\" align=\"right\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">". formatPositif($surplusDefisitLalu) ."</td>
                                                     </tr>";
				...
				case 9:
					$cRet .= "<tr>
													<td valign=\"top\"  width=\"5%\" align=\"center\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">$kd_rek</td>
													<td valign=\"top\"  width=\"65%\"  align=\"left\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">$nama</td>
													<td valign=\"top\"  width=\"15%\" align=\"right\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">". formatPositif($ekuitasAkhir) ."</td>
													<td valign=\"top\"  width=\"15%\" align=\"right\" style=\"font-size:12px;font-family:Arial;border-bottom:none;border-top:none\">$c1" . number_format($sal_awal, "2", ",", ".") . "$d1</td>
													</tr>";
			}
		}
	...
	function rpt_neraca_pemda($cbulan = "", $cetak = 1, $tglttd = "", $ttd = "")
	{
		...
		$queryneraca = " SELECT parent, kode, uraian, seq, isnull(normal,'') as normal, isnull(kode_1,'xxx') as kode_1, isnull(kode_2,'xxx')  as kode_2, isnull(kode_3,'xxx') as kode_3, 
										isnull(kode_4,'xxx') as kode_4, isnull(kode_5,'xxx') as kode_5, isnull(kode_6,'xxx') as kode_6, isnull(kode_7,'xxx') as kode_7, 
										isnull(kode_8,'xxx') as kode_8, isnull(kode_9,'xxx') as kode_9, isnull(kode_10,'xxx') as kode_10, isnull(kode_11,'xxx') as kode_11,
										isnull(kode_12,'xxx') as kode_12, isnull(kode_13,'xxx') as kode_13, isnull(kode_14,'xxx') as kode_14, isnull(kode_15,'xxx') as kode_15 
		...
		$nilaiAset = formatPositif(($assetTetap->nilai + $asset->nilai),2,',','.');
		...
		$queryKewajiban = "SELECT 
					...
					WHERE
						...
						OR ( trh.no_voucher LIKE '%-LO-NERACA-%' AND LEFT ( trd.kd_rek6, 4 ) = 2106  )";
		$queryKewajibanLalu = "SELECT SUM(kredit) AS kredit, SUM(debet) AS debet FROM trdju_pkd AS trd INNER JOIN trhju_pkd AS trh ON trh.kd_skpd = trd.kd_unit AND trh.no_voucher = trd.no_voucher WHERE LEFT(trd.kd_rek6,2) = ? AND YEAR(trh.tgl_voucher) = ?";
		$kewajiban = $this->db->query($queryKewajiban,[2,$thn_ang,$bulan])->row();
		...
		foreach ($query10->result_array() as $res) {
			...
			$normal = $res['normal'];
			$parent = $res['parent'];
			$kode = trim($res['kode_1']);

			$konversiLra = substr($kode,0,4);
			if (($konversiLra >= 1301 && $konversiLra < 1306)) {
				...
				$querySaldo = "SELECT SUM(trd.debet) AS debet, SUM(trd.kredit) AS kredit FROM trhju_pkd AS trh
					...
					WHERE LEFT(trd.kd_rek6,$length) = ? AND YEAR(trh.tgl_voucher) = ? AND MONTH(trh.tgl_voucher)  <= ? AND trd.kd_rek6 NOT IN ('520399999999','520288888888','520299999999','520388888888','520508010005','520499999999', '5488888888') 
					OR (LEFT(trd.kd_rek6, $length) = $kode AND trh.no_voucher LIKE '%-LO-NERACA-%')
				";
			} elseif($kode == 2106) {
				$querySaldo = "SELECT
					SUM(debet) as debet,
				...
				WHERE
					...
					trh.no_voucher LIKE '%-LO-NERACA-%' AND LEFT(trd.kd_rek6, 4) = ? AND YEAR(trh.tgl_voucher) = ? AND MONTH(trh.tgl_voucher) <= ?";
			}  else {
				$querySaldo = "SELECT SUM(kredit) AS kredit, SUM(debet) AS debet FROM trdju_pkd AS trd INNER JOIN trhju_pkd AS trh ON trh.kd_skpd = trd.kd_unit AND trh.no_voucher = trd.no_voucher WHERE LEFT(trd.kd_rek6,len($kode)) = ? AND YEAR(trh.tgl_voucher) = ? AND MONTH(trh.tgl_voucher) <= ? ";
			}

			$querySaldoAwal = "SELECT SUM(kredit) AS kredit, SUM(debet) AS debet FROM trdju_pkd AS trd INNER JOIN trhju_pkd AS trh ON trh.kd_skpd = trd.kd_unit AND trh.no_voucher = trd.no_voucher WHERE LEFT(trd.kd_rek6,len($kode)) = ? AND YEAR(trh.tgl_voucher) = ? ";
		
			if ($kode != 'xxx' ) {
				$saldoAwal = $this->db->query($querySaldoAwal,[$kode, $thn_ang_1])->row();
				if ($konversiLra >= 1301 && $konversiLra < 1306) {
					$lra =  "52".substr($kode,2);
					$saldo = $this->db->query($querySaldo,[$lra, $thn_ang,$bulan])->row();
				} else  {
					$saldo = $this->db->query($querySaldo,[$kode, $thn_ang,$bulan])->row();
				}
				$saldoLalu = substr($kode,0,1) == 1 ? $saldoAwal->debet - $saldoAwal->kredit :  $saldoAwal->kredit - $saldoAwal->debet;
			...
			$cRet    .= "<tr>
							...
							<td style=\"font-size:12px;font-family:Arial;vertical-align:top;border-top: solid 1px black;border-bottom: none;\" width=\"5%\">$kode</td>                                     
		...
		foreach ($query10->result_array() as $key => $res) {
			...
			if (($konversiLra >= 1301 && $konversiLra < 1306)) {
				...
				$query = "SELECT SUM(trd.debet) AS debet, SUM(trd.kredit) AS kredit FROM trhju_pkd AS trh
					INNER JOIN trdju_pkd AS trd ON trd.kd_unit = trh.kd_skpd AND trd.no_voucher = trh.no_voucher
					WHERE LEFT(trd.kd_rek6,$length) = ? AND YEAR(trh.tgl_voucher) = ? AND MONTH(trh.tgl_voucher)  <= ? AND trd.kd_rek6 NOT IN ('520399999999','520288888888','520299999999','520388888888','520508010005','520499999999', '5488888888') 
					OR (LEFT(trd.kd_rek6, $length) = $kode_1 AND trh.no_voucher LIKE '%-LO-NERACA-%')
				";
				...
				$q = $this->db->query("SELECT 
					...
					WHERE
						...
						AND MONTH ( trh.tgl_voucher ) <= ? 
						AND YEAR ( trh.tgl_voucher ) = ? 
						OR ( trh.no_voucher LIKE '%-LO-NERACA-%' AND LEFT ( trd.kd_rek6, 4 ) = 2106  )",[$kode_1, $xbulan, $thn_ang]);
				...
				$q = $this->db->query(" SELECT SUM(b.debet) AS debet,SUM(b.kredit) AS kredit from $trhju a inner join $trdju b on a.no_voucher=b.no_voucher 
									and b.kd_unit=a.kd_skpd where (YEAR(a.tgl_voucher) = $thn_ang AND MONTH(a.tgl_voucher) <=  $xbulan) and
										(kd_rek6 like '$kode_1%' or kd_rek6 like '$kode_2%'  or 
										kd_rek6 like '$kode_3%' or kd_rek6 like '$kode_4%'  or 
										kd_rek6 like '$kode_5%' or kd_rek6 like '$kode_6%'  or 
										kd_rek6 like '$kode_7%' or kd_rek6 like '$kode_8%'  or 
	...
	function rpt_neraca_pemda_unit_obyek_skpd($cbulan = "", $kd_skpd = "", $cetak = 1, $tglttd = "", $ttd = "")
	{
		...
		// Hitung asset tetap
		$assetTetap = $this->db->query("EXEC assets_tetap_skpd ?,?,? ", [$bulan,2023,$kd_skpd])->row();
		...
		$querySaldoAwalEkuitas = "SELECT SUM
					( kredit - debet ) AS nilai 
				FROM
					trhju_pkd AS trh
					INNER JOIN trdju_pkd AS trd ON trd.kd_unit = trh.kd_skpd 
				...
				WHERE
					trh.kd_skpd = ?
					AND LEFT ( trd.kd_rek6, 4 ) IN (3101,3103) 
					AND YEAR ( trh.tgl_voucher ) = ? 
					AND trh.tabel= 1 
					AND reev IN (0,3) 
					AND nm_rek6 NOT LIKE '%Surplus/Defisit-LO%'";
		$saldoAwalEkuitas = $this->db->query($querySaldoAwalEkuitas,[$kd_skpd,$thn_ang_1])->row();
        ...
        // selisih surplus non operasional dan defisit operasion
        $surplusDefisit = $surplusTahunLalu + $nonOperasiTahunLalu;
		$surplusDefisitIni = $surplusTahunIni + $nonOperasiTahunIni;
		...
		$queryKewajiban = "SELECT 
					...
					WHERE
						trh.kd_skpd = ?
						AND LEFT ( trd.kd_rek6, 1 ) =?
						AND LEFT(trd.kd_rek6,4) != 2106
						AND YEAR ( trh.tgl_voucher ) = ?
						AND MONTH ( trh.tgl_voucher ) <= ? 
						OR ( trh.kd_skpd = '$kd_skpd' AND trh.no_voucher LIKE '%-LO-NERACA-%' AND LEFT ( trd.kd_rek6, 4 ) = 2106  )";
		$queryKewajibanLalu = "SELECT SUM(kredit) AS kredit, SUM(debet) AS debet FROM trdju_pkd AS trd INNER JOIN trhju_pkd AS trh ON trh.kd_skpd = trd.kd_unit AND trh.no_voucher = trd.no_voucher WHERE trh.kd_skpd = ? AND LEFT(trd.kd_rek6,2) = ? AND YEAR(trh.tgl_voucher) = ?";
		$kewajiban = $this->db->query($queryKewajiban,[$kd_skpd,2,$thn_ang,$bulan])->row();
		$kewajibanLalu = $this->db->query($queryKewajibanLalu,[$kd_skpd,21,2022])->row();
		$saldoKewajiban = $kewajiban->kredit - $kewajiban->debet;
		$saldoKewajibanLalu = $kewajibanLalu->kredit - $kewajibanLalu->debet;


		$saldoAwalEkuitas = $surplusDefisit + $saldoAwalEkuitas->nilai;

		$saldoAwalKewajibanEkuitas = $saldoAwalEkuitas;
		$saldoAkhirKewajibanEkuitas = 0;
		...
		foreach ($query10->result_array() as $key => $res) {
			...
			if ($kode != 'xxx') {
				...
				$querySaldoAwal = "SELECT SUM(debet) AS debet, SUM(kredit) AS kredit FROM trhju_pkd AS trh INNER JOIN trdju_pkd AS trd ON
				trd.no_voucher = trh.no_voucher AND trh.kd_skpd = trd.kd_unit WHERE trh.kd_skpd = ? AND YEAR(trh.tgl_voucher) = ? AND LEFT(trd.kd_rek6,len($kode)) = ? ";
				$konversiLra = substr($kode,0,4);
				if (($konversiLra >= 1301 && $konversiLra < 1306)) {
					$length = strlen($kode);
					$querySaldo = "SELECT SUM(trd.debet) AS debet, SUM(trd.kredit) AS kredit FROM trhju_pkd AS trh
					...
					$querySaldo = "SELECT SUM(kredit) AS kredit, SUM(debet) AS debet FROM trdju_pkd AS trd INNER JOIN trhju_pkd AS trh ON trh.kd_skpd = trd.kd_unit AND trh.no_voucher = trd.no_voucher WHERE trh.kd_skpd = ? AND LEFT(trd.kd_rek6,len($kode)) = ? AND YEAR(trh.tgl_voucher) = ? AND MONTH(trh.tgl_voucher) <= ? ";
				}
				...
				if($kode == 2 && $parent == 1) {
					$saldoAkhirKeawijiban = $nilaiAwal + $saldoKewajiban;

					$saldoAwalKewajibanEkuitas += $nilaiAwal;
					$saldoAkhirKewajibanEkuitas += $saldoAkhirKeawijiban;
				}

				if ($kode == 1 && $parent == 1) {
					$tahunIni = $nilaiAset;
					$tahunLalu = formatPositif($nilaiAwal);
				} else if($parent == 2 && $kode == 13) {
					$tahunIni = formatPositif($assetTetap->nilai);
					$tahunLalu = formatPositif($nilaiAwal) ;
				} else if($kode == 3101)  {
					$temp = ($assetTetap->nilai + $asset->nilai) - $saldoAkhirKeawijiban;
					$saldoAkhirKewajibanEkuitas += $temp;
					$tahunIni = formatPositif($temp);
					$tahunLalu = formatPositif($saldoAwalEkuitas);
				} elseif(in_array($kode,[2,21])) {
					$tahunIni = formatPositif($saldoAkhirKeawijiban);
					$tahunLalu = formatPositif($nilaiAwal);
				} else {
					$tahunIni = formatPositif($nilaiAkhir);
					$tahunLalu = formatPositif($nilaiAwal);
				}
			...
			$cetakKode = $kode != "xxx" ? $kode : "";

			$cRet    .= "<tr>
							<td style=\"font-size:12px;font-family:Arial;vertical-align:top;border-top: solid 1px black;border-bottom: none;$bold[$parent]\" width=\"10%\" align=\"center\">$no</td>                                     
							<td style=\"font-size:12px;font-family:Arial;vertical-align:top;border-top: solid 1px black;border-bottom: none;$bold[$parent]\" width=\"5%\">$cetakKode</td>                                     
							<td style=\"font-size:12px;font-family:Arial;vertical-align:top;border-top: solid 1px black;border-bottom: none;$padding[$parent]$bold[$parent]\" width=\"60%\">$uraian</td>
							<td style=\"font-size:12px;font-family:Arial;vertical-align:top;border-top: solid 1px black;border-bottom: none;$bold[$parent]\" width=\"20%\" align=\"right\">$tahunIni</td>
							<td style=\"font-size:12px;font-family:Arial;vertical-align:top;border-top: solid 1px black;border-bottom: none;$bold[$parent]\" width=\"20%\" align=\"right\">$tahunLalu</td>
						</tr>";
		}
