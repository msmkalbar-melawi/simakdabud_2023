...
class SP2BBludController extends CI_Controller {
    ...
    function index() 
    {
        ...
        $this->template->load('template','tukd/blud/index',$data) ; 
    }    
   ...
   $query1 = $this->db->query("select COUNT(number_sp2b) as nomor from trhsp3b_blud where number_sp2b not in ('0')");
    ...
    function skpd_rsud() {
		//$kd_skpd = $this->session->userdata('kdskpd');
        $sql = "SELECT kd_skpd,nm_skpd FROM ms_skpd where kd_skpd = '1.02.0.00.0.00.01.0001'";
        $query1 = $this->db->query($sql);  
        $test = $query1->num_rows();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
